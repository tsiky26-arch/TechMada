...
<title>TechMada RH - Gestion Types de Conge</title>
...
<div class="app">
  <aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <a class="active" href="<?= site_url('admin/leave-types') ?>"><i class="bi bi-tags"></i> Types de conge</a>
  ...
  <main class="main">
    <header class="topbar">
      <div>
        <div class="title">Gestion des types de conge</div>
    ...
    <section class="content">
      ...
      <div class="card">
        ...
        <form method="post" action="<?= site_url('admin/leave-types') ?>" class="filters">
          <div class="field">
            <label>Libelle</label>
          ...
          <div class="field">
            ...
            <input name="jours_annuels" type="number" step="0.5" min="0" required>
          </div>
          <div class="field">
            <label>Deductible (affecte le solde)</label>
            <select name="deductible" required>
              <option value="1">Oui</option>
              <option value="0">Non</option>
            </select>
          </div>
          <button class="btn" type="submit"><i class="bi bi-plus-circle"></i> Ajouter type de conge</button>
        </form>
      </div>

      <div class="card">
        <h2>Liste des types de conge</h2>
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              <?php if (empty($types)): ?>
                <tr><td colspan="4" class="empty">Aucun type de conge.</td></tr>
              <?php else: ?>
                <?php foreach ($types as $type): ?>
                  <tr>
                    <td><?= esc((string) $type['id']) ?></td>
                    <td><?= esc($type['libelle']) ?></td>
                    <td><?= esc(number_format((float) $type['jours_annuels'], 1, ',', ' ')) ?></td>
