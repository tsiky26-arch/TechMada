...
<div class="app">
  <aside class="sidebar">
    ...
    <nav class="nav">
      <a href="<?= site_url('dashboard') ?>"><i class="bi bi-grid-1x2"></i> Tableau de bord</a>
      <a class="active" href="<?= site_url('rh') ?>"><i class="bi bi-people"></i> Espace RH</a>
      <a href="<?= site_url('rh') ?>?statut=en_attente"><i class="bi bi-hourglass-split"></i> En attente</a>
      <a href="<?= site_url('rh') ?>?statut=approuve"><i class="bi bi-check2-circle"></i> Approuvees</a>
    </nav>
  ...
  <main class="main">
    <header class="topbar">
      ...
      <a class="btn btn-light" href="<?= site_url('dashboard') ?>"><i class="bi bi-arrow-left"></i> Retour</a>
    </header>
    ...
    <section class="content">
      ...
      <div class="card">
        <form class="filters" method="get" action="<?= site_url('rh') ?>">
          <div class="field">
          ...
          <a class="btn btn-light" href="<?= site_url('rh') ?>"><i class="bi bi-arrow-counterclockwise"></i> Reinitialiser</a>
        </form>
      </div>
                ...
                <tr>
                  ...
                  <td>
                    <?php if ($demande['statut'] === 'en_attente'): ?>
                      <div class="actions">
                        <form class="inline-form" method="post" action="<?= site_url('rh/conges/' . (string) $demande['id'] . '/approuver') ?>">
                        ...
                        <form class="inline-form" method="post" action="<?= site_url('rh/conges/' . (string) $demande['id'] . '/refuser') ?>">
                          <input type="hidden" name="departement_id" value="<?= esc($filters['departement_id']) ?>">
                          <input type="hidden" name="statut" value="<?= esc($filters['statut']) ?>">
                          <div class="field inline-comment">
                    ...
                    <?php elseif ($demande['statut'] === 'approuve'): ?>
                      <form class="inline-form" method="post" action="<?= site_url('rh/conges/' . (string) $demande['id'] . '/annuler') ?>">
                        <input type="hidden" name="departement_id" value="<?= esc($filters['departement_id']) ?>">
                        <input type="hidden" name="statut" value="<?= esc($filters['statut']) ?>">
                        <div class="field inline-comment">
                          <textarea name="commentaire_rh" placeholder="Commentaire optionnel"></textarea>
